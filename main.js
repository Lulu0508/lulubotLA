...
client.on("ready", () => {
    ...
    .addField("Derzeitige Uhrzeit", new Date())
    .addField("Mein Username + Tag", `${client.user.username}#${client.user.discriminator}`)
    .addField("Owner des Bots", `${client.users.get("373857433380061184").username}#${client.users.get("373857433380061184").discriminator}`)
    .addField("Willkommmen", "Ich Freue mich ihnen Helfen zu können")
...
function cmd_roboter(msg, args){
    if(args[0] == undefined) return msg.channel.send("Wie soll den dein Robotor heißen?")
    let embed = new Discord.RichEmbed()
    .setDescription(`So würde ein Robotor mit dem namen: ${args.join(" ")} ausehen`)
...
function cmd_binlulu(msg, args){
    if(msg.author.id === config.owner){
        ...
        msg.channel.send("Sicher?")
    }
}
function cmd_fortnite_s(msg, args){
   let username = args[0]
...
function cmd_id(msg, args){
    ...
    .setTitle("ID'S")
    .setColor("#e03e16")
    .addField("Deine User ID", msg.author.id)
    .addField("Aktuelle Server ID", msg.guild.id)
    .addField("Aktuelle Channel ID", msg.channel.id)
    msg.channel.send(idembed)
}
function cmd_shop(msg, args){
    let shopembed = new Discord.RichEmbed()
    .setTitle("TheRealShop")
    .setDescription("LuluArmy")
    .setThumbnail("https://www.welt.de/img/vermischtes/mobile166641813/3792501637-ci102l-w1024/CRESTED-BLACK-MACAQUE.jpg")
    .addField(`Premium
Preis: 400LC`, "Beschreibung: Joa Premium halt lölz")
    .setFooter("Kaufen mit #buy", "https://www.welt.de/img/vermischtes/mobile166641813/3792501637-ci102l-w1024/CRESTED-BLACK-MACAQUE.jpg")
    .setColor("#e7ea3c")
    msg.channel.send(shopembed)
}
async function cmd_test(msg, args){
    let coinsu = coins[msg.author.id].coins;
    if(!coins[msg.author.id]){
        coins[msg.author.id] = {
            coins:0
        };
    }
    if(coinsu < 10) return msg.channel.send("nicht genug coins!")
    
       
        msg.member.addRole("435496705962672154")
        coins[msg.author.id] ={coins: coinsu - 10};
        fs.writeFile("./coins.json", (coins))
    let testembed = new Discord.RichEmbed()
    .setTitle("Rang Gekauft(Owner)")
    .setColor("#ea85af")
    .addField("LuluCoins abgezogen: 10", `noch vorhandene coins: ${coinsu}`)
    msg.channel.send(testembed)

  
    
}
function cmd_buy(msg, args){
    let args0 = args[0]
    if(!args0) return msg.channel.send("Was willst du denn Kaufen")
    if(args0 === "Premium"){
    if(msg.guild.id != "415889442222505985"){
        msg.channel.send({embed: {
            title: "Sorry",
            description: "(WIRKLICH..... SORRY :heart:)",
            fields: [{
                name: "Leider gibt es diesen Rang",
                value: "nur auf den (Lulu Server)[https://discord.gg/qQQEggA]"
            }]
        }})
        }else
        if(msg.guild.id === "415889442222505985"){  
            ...
            if(coinsu < 400) return msg.channel.send("nicht genug coins!")
            
               
                msg.member.addRole("458313934227636234")
                coins[msg.author.id] ={coins: coinsu - 400};
                fs.writeFile("./coins.json", (coins))
            let testembed = new Discord.RichEmbed()
            .setTitle("Rang Gekauft: Premium")
            .setColor("#ea85af")
            .addField("LuluCoins abgezogen: 400LC", `noch vorhandene coins: ${coinsu}`)
            msg.channel.send(testembed)
        }
}}
...
function cmd_pay(msg, args){
    if(!coins[msg.author.id]) return msg.reply("Du hast keine Coins!")

 let pUser = msg.guild.member(msg.mentions.members.first()) || msg.guild.members.get(args[0])
 ...
 if(sCoins < args[0]) return msg.reply("Du hast nicht genug Coins!");
 ...
 msg.channel.send(`${msg.author} hat ${pUser} ${args[1]} LuluCoins gegeben!`)
...
function cmd_coins(msg, args){
    let lol = msg.mentions.members.first();
    if(!lol){
    if(!coins[msg.author.id]){
        coins[msg.author.id] = {
            coins:0
...
.setColor("#c815d8")
.addField("LuluCoins", ucoins);

msg.channel.send(coinembed)} else
if(lol){
    ...
    let ucoins = coins[lol.id].coins;
    let coinembed = new Discord.RichEmbed()
    .setAuthor(`${lol.user.username}`)
    .setColor("#c815d8")
    .addField("LuluCoins", ucoins);
...
function cmd_msg(msg, args){
 let channelid = args[0]
    ...
    client.channels.get(`${channelid}`).send(`[${msg.guild.name}] [${msg.author.username}]: ${text}`)}
...
function cmd_userinfo(msg, args){
    ...
    let userinfoMEMBER = msg.mentions.members.first()

  if(!userinfoMEMBER){
        let EMBED = new Discord.RichEmbed()

    .setTitle(`Userinfo von ${msg.author.username}`)
    .setThumbnail(msg.author.avatarURL)
    .setTimestamp(new Date())
    .setFooter(`Command genutzt von: ${msg.author.username}`, msg.author.avatarURL)
    .setColor(`#96e059`)
    .addField(`Name`, `${msg.author.username}#${msg.author.discriminator}(ID: ${msg.author.id})`)
    if(msg.author.username == msg.member.displayName)
    EMBED.addField(`Nickname`, `Kein Nickname`)
    .addField(`Beigetreten am`, msg.guild.joinedAt)
    .addField(`Du hast deinen Account erstellt am`, msg.author.createdAt)
    .addField(`Rollen`, msg.member.roles.array().slice(1).map(r => `${r.name}`).join(', '))    
    if(msg.author.username != msg.member.displayName)
    EMBED.addField(`Nickname`, msg.member.displayName)
    .addField(`Beigetreten am`, msg.guild.joinedAt)
    .addField(`Du hast deinen Account erstellt am`, msg.author.createdAt)
    .addField(`Rollen`, msg.member.roles.array().slice(1).map(r => `${r.name}`).join(', '))    
   ...
   if(userinfoMEMBER){
    ...
    .setTitle(`Userinfo von ${userinfoMEMBER.user.username}`)
    .setThumbnail(userinfoMEMBER.user.avatarURL)
    .setTimestamp(new Date())
    .setFooter(`Command genutzt von: ${msg.author.username}`, msg.author.avatarURL)
    .setColor(`#96e059`)
    .addField(`Name`, `${userinfoMEMBER.user.username}#${userinfoMEMBER.user.discriminator} (ID: ${userinfoMEMBER.user.id})`)
    if(userinfoMEMBER.user.username == userinfoMEMBER.displayName)
    EMBED.addField(`Nickname`, `Kein Nickname`)
    
    .addField(`Account erstellt am`, userinfoMEMBER.user.createdAt)
    .addField(`Rollen`, userinfoMEMBER.roles.array().slice(1).map(r => `${r.name}`).join(', '))    
    if(userinfoMEMBER.user.username != userinfoMEMBER.displayName)
    EMBED.addField(`Nickname`, userinfoMEMBER.displayName)
...
function cmd_calculate(msg, args){
    args[0] + args[1] == ergebniss
    msg.channel.send(ergebniss)
...
function cmd_nv(msg, args) {
    Embeds.grün(msg.channel, `Hi kann in nem Embed kästchen schreiben obwohl ich kein bot bin lol(mein secret trick :-) )`)
    msg.delete()
}
...
let coinAmt = Math.floor(Math.random() * 1) + 5;
 let baseAmt = Math.floor(Math.random() * 1) + 5;
 if(coinAmt === baseAmt){
     coins[message.author.id] = {
